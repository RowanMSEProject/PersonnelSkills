...
public class LoginFacadeREST extends AbstractFacade<Login> {
    @PersistenceContext(unitName = "JurkPU")
    ...
    public void createUser(@FormParam("username") String username,
        ...
        if(checkPassword(password)) {
            List<Login> list =em.createNamedQuery("Login.findAll").getResultList();
            int id=Integer.parseInt(list.get(list.size()-1).getUserid());
            id++;
